...
describe('BarcodeScanner — REQ-BOOK-009 (S11): 디바운스 (컴포넌트 보조 테스트)', () => {
  // @MX:NOTE: [AUTO] 본 describe 블록은 컴포넌트 수준 보조 테스트.
  //           setScanning(false)(S10 카메라 중지) 로 인해 디바운스 분기(shouldSuppressDuplicate)
  //           가 단일 스캔 흐름에서 도달 불가하므로, 첫 스캔 1회 호출만 검증 가능.
  //           REQ-BOOK-009 디바운스 계약 자체는 src/features/book/__tests__/debounce.test.ts
  //           의 순수 함수 단위 테스트가 검증한다 (동일 ISBN 2초 내 억제/경과 후 재허용/다른 ISBN 허용).

  beforeEach(() => {
    setPermissionResponse(grantedPermission);
  });

  // S11 보조: 단일 스캔 시 1회 호출 (카메라 중지로 자연스럽게 중복 방지)
  it('단일 ISBN 스캔 시 콜백을 정확히 1회 호출한다', () => {
    const onIsbnDetected = jest.fn();
    renderScanner({ onIsbnDetected });
    act(() => {
      simulateBarcodeScan('ean13', '9788966262335');
  ...
  // S11 보조: 카메라 중지로 인해 두 번째 스캔은 발생하지 않음 (정상 동작)
  it('첫 스캔 후 카메라가 중지되어 두 번째 스캔은 콜백을 발생시키지 않는다', () => {
    const onIsbnDetected = jest.fn();
    renderScanner({ onIsbnDetected });
    act(() => {
      simulateBarcodeScan('ean13', '9788966262335');
    });
    // 두 번째 시도 — scanning=false 로 인해 handleBarcodeScanned 진입 즉시 return
    act(() => {
      simulateBarcodeScan('ean13', '9788966262335');
    });
    expect(onIsbnDetected).toHaveBeenCalledTimes(1);
