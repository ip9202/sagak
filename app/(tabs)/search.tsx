 ...
 * - onSelectBook(result) → resolveBookId(isbn) 로 UUID 획득 후 router.push(`/${UUID}`)
 *
 * SPEC-LIBRARY-001 변경점: 검색 결과의 ISBN 을 직접 라우팅하지 않고,
 * books.id(UUID) 로 해소한 뒤 /<UUID> 로 이동한다. 미등록 ISBN 인 경우
 * NOT_FOUND 가 발생하며, 사용자에게 책 등록 플로우 안내 메시지를 노출한다.
 *
 * @MX:NOTE: [AUTO] 라우트 경로는 /(tabs)/[bookId] 그룹 라우트 기준 /<UUID> — (tabs) route group 은 URL 에서 생략된다. /book/<UUID> 로 push 하면 unmatched route 발생 (PR fix).
 *
