...
포함 키: `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, `ENV`
...
`EXPO_PUBLIC_SUPABASE_ANON_KEY` 중 하나라도 비어 있으면
...
### 6.1 Supabase 프로젝트 ref 확보

> **단일 프로젝트 정책 (2026-07-01 결정)**: dev 환경은 로컬 Docker Supabase, staging/prod 환경은 단일 클라우드 프로젝트(lqltwbpocbgoxvhlmjdo)를 공유한다. staging/prod 구분은 ENV flag 로만 수행한다. 별도 클라우드 프로젝트 생성은 MVP 이후 검토.

**dev 환경 (로컬 Docker)**:
- 로컬 Docker Compose로 Supabase 인스턴스 실행
- `SUPABASE_DEV_PROJECT_REF`는 사용하지 않음 (로컬 환경)

**staging/prod 환경 (단일 클라우드 프로젝트)**:
1. Supabase Dashboard > **Project Settings > API** 로 이동
2. **Reference ID** 값 복사 (또는 Project URL `https://<ref>.supabase.co` 에서 `<ref>` 부분 추출)
3. staging/prod 모두 동일한 ref를 `SUPABASE_STAGING_PROJECT_REF`, `SUPABASE_PROD_PROJECT_REF`로 설정

> **참고**: staging/prod가 단일 클라우드 프로젝트를 공유하므로, 데이터 격리는 RLS(Row Level Security) 정책에 의존한다.
...
### 6.5.1 process-join-request 보안 게이트 (SPEC-SECURITY-001, PR #121)
...
2. **jose 서명 검증 (앱 단)** — `verifyAndExtractJwtSub`(ES256/JWKS) 가 게이트웨이와 무관하게 JWT 서명을 독립 검증. 게이트웨이 verify_jwt 가 우회되어도 서명 위조 시 401.

> A1 CI job 은 현재 develop 룰셋의 **필수 상태 체크가 아님** (ops 후속 — 룰셋 등록 필요). PR 머지 자체는 A1 통과를 요구하지 않으므로 수동 확인 권장.

**배포 게이트 (프로덕션 배포 전 필수)**:

process-join-request 를 프로덕션에 배포하기 전, **로컬 smoke test** 로 서명 검증 동작을 확인한다. alg/iss/aud/JWKS-URL 의 정확성은 런타임에만 검증 가능하기 때문. (확정값: alg=ES256, iss=`${SUPABASE_URL}/auth/v1`, aud=`authenticated` — 2026-07-03 REQ-SEC-050 smoke)
...
| `Invalid access token format` | `SUPABASE_ACCESS_TOKEN`이 `sbp_...` 형식이 아님 | 6.2절에서 재발급. Supabase Dashboard 에서만 생성 가능 |
| `ERROR: SUPABASE_DEV_PROJECT_REF is not set` | `ENV=development` 이지만 `SUPABASE_DEV_PROJECT_REF` 미설정 | 6.3절 확인. 로컬 `.env` 또는 GitHub Secrets 에 ref 등록 |
| `ERROR: SUPABASE_STAGING_PROJECT_REF is not set` | `ENV=staging` 이지만 `SUPABASE_STAGING_PROJECT_REF` 미설정 | 6.3절 확인. GitHub Secrets 에 staging ref 등록 |
| `ERROR: SUPABASE_PROD_PROJECT_REF is not set` | `ENV=production` 이지만 `SUPABASE_PROD_PROJECT_REF` 미설정 | 6.3절 확인. GitHub Secrets 에 production ref 등록 |
| `supabase: command not found` | Supabase CLI 미설치 | `brew install supabase/tap/supabase` 실행 (v2.104.0 이상) |
| `Error deploying function: 401 Unauthorized` | `SUPABASE_ACCESS_TOKEN` 만료 또는 잘못됨 | 6.2절에서 재발급. Token 유효기간 확인 |
| `Error: Project not found` | PROJECT_REF 가 프로젝트와 일치하지 않음 | 6.1절에서 ref 재확인. Dashboard 의 Reference ID 와 정확히 일치해야 함 |
...
- `.github/workflows/deploy.yml` — CI 배포 자동화 (release/hotfix 트리거, EAS prod 빌드, hotfix dual-merge 검증)
