...
3. **제품 설정 > 카카오 로그인 > Redirect URI** 에 다음 등록:
   ```
   sagak://auth/callback
   ```
...
4. **승인된 리디렉션 URI**(Web 클라이언트 항목)에 다음 등록:
   ```
   sagak://auth/callback
   ```
...
4. **Callback URL** (Supabase 가 표시하는 값) 을 Kakao 콘솔 Redirect URI 에 **추가로** 등록:
   ```
...
4. Supabase 가 표시하는 Callback URL 을 Google Console 에 **추가로** 등록:
   ```
   https://<project-ref>.supabase.co/auth/v1/callback
   ```
...
3. Callback URL(`https://<project-ref>.supabase.co/auth/v1/callback`)을
   Naver 콘솔에 추가 등록
