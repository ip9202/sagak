...
> 프로젝트의 secret 보호 컨벤션은 `src/config/__tests__/credential-hygiene.test.ts` 를 참조한다.
...
4. **Token scopes** (필수):
   - `project:releases` — release 생성 및 sourcemap 업로드
   - `project:write` — 프로젝트에 파일 업로드
   - `org:read` — 조직 정보 읽기
   > **정확한 scope 명칭은 Sentry 버전에 따라 변경될 수 있으므로 [Auth Tokens 공식 문서](https://docs.sentry.io/account/auth-tokens/) 를 확인한다.
...
`.env.staging` (및 production 프로젝트 생성 후 `.env.production`)의 `EXPO_PUBLIC_SENTRY_DSN` 값을 채운다.

```bash
# .env.staging
EXPO_PUBLIC_SUPABASE_URL=https://<project-ref>.supabase.co
EXPO_PUBLIC_SUPABASE_ANON_KEY=<your-anon-key>
EXPO_PUBLIC_SENTRY_DSN=https://examplePublicKey@o0.ingest.sentry.io/123456
ENV=staging
```

```bash
# .env.production (production 프로젝트 생성 후)
EXPO_PUBLIC_SUPABASE_URL=https://<project-ref>.supabase.co
EXPO_PUBLIC_SUPABASE_ANON_KEY=<your-anon-key>
EXPO_PUBLIC_SENTRY_DSN=https://examplePublicKey@o0.ingest.sentry.io/789012
ENV=production
```

> **placeholder 커밋 규칙**: `.env.staging` / `.env.production` 은 `.gitignore` 예외로 커밋되지만,
> **DSN 값은 빈 문자열(`EXPO_PUBLIC_SENTRY_DSN=`)로 커밋하고 실제 값은 로컬 `.env`(gitignored) 또는
> CI/EAS 빌드 시점에 secret 으로 주입**하는 것을 권장한다.
> EAS 빌드에서는 EAS Secret(`eas secret:create`)으로 `EXPO_PUBLIC_SENTRY_DSN` 을 등록할 수 있다.
