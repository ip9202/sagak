...
### 2.0 모든 snippet은 service_role/BYPASSRLS 문맥에서 실행됨 (CRITICAL)

SQL Editor로 실행하는 모든 snippet(00~08)은 service_role/postgres 권한으로 동작하며 **RLS가 적용되지 않는다(BYPASSRLS)**. 즉 WHERE 절 누락·오타가 전역 파괴적 효과를 갖는다(예: `DELETE`의 `user_id` 조건 누락 시 전 사용자 행 삭제). **각 MUTATING snippet 실행 전 반드시 대응하는 SELECT로 대상 행을 먼저 확인**할 것. (백업 테이블만 C1 fix로 RLS FORCE가 적용되어 있으나, 본체 user_books는 service_role에 의해 우회된다.)
...
>
> **⚠️ 민감 데이터 취급 (S2)**: `per_user_breakdown` 결과는 `user_id` + `book_id` 조합의 민감 정보를 포함한다. 화면 공유·스크린샷을 금지하며, **count 값(`multi_reading_user_count`, `cleanup_target_row_count`)만 기록**한다. 상세 행 목록이 필요한 경우에도 개인 식별 값은 마스킹하여 비공개 채널에만 보관한다.
...
```sql
-- C1 검증: 백업 테이블에 RLS가 적용되었는지 확인 (both must be true)
SELECT c.relname,
       c.relrowsecurity AS rls_enabled,
       c.relforcerowsecurity AS rls_forced
FROM pg_class c
WHERE c.relname = 'backup_enforce_single_reading';
-- 기대: rls_enabled = true, rls_forced = true
```
...
3. **트랜잭션 주의 (W5)**: Supabase SQL Editor는 명시적 `BEGIN`이 없으면 각 statement를 **autocommit**으로 실행한다 (전체가 한 트랜잭션이 아님). 따라서 Step 1(정리 UPDATE)이 커밋된 후 Step 2(인덱스 생성)가 실패하면 "정리는 됐으나 인덱스 없음" 상태가 잔류할 수 있다. **권고**: 마이그레이션 원본 수정 없이, SQL Editor에서 `BEGIN;` 으로 시작해 `<마이그레이션 본문>` `COMMIT;` 으로 감싸서 실행한다. 실패 시 `ROLLBACK;` 으로 전체 되돌린다. (백업 테이블은 Phase 1에서 이미 생성되었으므로 롤백해도 보존된다.)
...
> **전제**: dev Phase 0~5가 모두 통과되었을 것. dev 프로젝트 ref: `lqltwbpocbgoxvhlmjdo`. prod 프로젝트 ref: `.env.production` 의 `EXPO_PUBLIC_SUPABASE_URL` 에서 추출하거나, Supabase Dashboard → Project Settings → General → **Reference ID** 에서 확인. dev(ref: `lqltwbpocbgoxvhlmjdo`)와 혼동 주의.
...
## 11. 백업 테이블 폐기 (Disposal)

prod 안정화 30일 후, 백업 테이블을 폐기한다.

```sql
-- 보존 기간(마이그레이션 안정화 30일) 경과 후 실행
DROP TABLE IF EXISTS public.backup_enforce_single_reading;
```

**폐기 전 확인 사항**:
- 삭제 전 해당 테이블에 **데이터 주체 삭제 요청 대상 행이 없는지** 반드시 확인한다 (개인정보 보호 의무).
- GDPR/PIPA 삭제 의무 충족을 위해 폐기 반드시 수행한다.

> **감사/추적성 (W6)**: 폐기 전까지 `backup_enforce_single_reading` 백업 테이블은 **어떤 행이 `shelved`로 전환되었는지에 대한 유일한 포렌식 기록**이다 (Step-1 정리가 비가역적이므로). 롤백 외에도 규제·감사 요청 시 행 단위 전환 이력을 제공할 수 있으므로, 보존 기간 중 임의 DROP을 금지한다.
