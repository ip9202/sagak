...
| SPEC-AUTH-001 | `useSession()` 훅 (세션 상태: loading/authenticated/unauthenticated) | 인증 가드에서 `useSession()` 소비하여 리다이렉트 분기 |
...
1. 인증 상태는 3가지로 분류된다 (SPEC-AUTH-001 `useSession()` 반환값 기준):
   - `loading` — 세션 복원 중 (스플래시/로딩 인디케이터 표시)
   - `authenticated` — 유효한 세션 존재 (`(tabs)` 그룹 진입)
   - `unauthenticated` — 세션 없음 (`(auth)` 그룹 진입)
...
**WHILE** 인증된 사용자가 `(auth)` 그룹 라우트(예: `/login`)에 직접 접근하려 하면,
...
**IF** `useSession()`이 `authenticated`이지만 온보딩(프로필 설정)이 미완료이면,
**THEN** 시스템은 홈 탭 대신 `(auth)` 그룹의 온보딩 화면으로 리다이렉트해야 한다 (pages_07 SCR-A04).

> 온보딩 완료 여부 판단 기준은 SPEC-AUTH-001이 세션 메타데이터로 제공한다 (예: `session.user.nickname` 존재 여부). 본 SPEC은 해당 메타데이터를 소비하기만 한다.
...
`useSession()`이 `loading` 상태일 때 표시할 스플래시 화면의 구체적 디자인(로고 애니메이션, 슬로건 등)은 pages_07 SCR-A01에 정의되어 있으나, 애니메이션 구현은 SPEC-UI-001 제외 범위(Reanimated 미도입)와 충돌한다.
