...
| 6  | 환경 분리 실제 구현 방식 (별도 Supabase 프로젝트 vs 단일 프로젝트 스키마 분리) | **부분 해결** | REQ-DEPLOY-023은 별도 프로젝트 가정, 비용 검토 후 단일 프로젝트+RLS 격리로 전환 가능 |
...
- v1.0.2(2026-06-19) 변경: 네이버 Custom OIDC auto-discovery → oauth2 모드 + naver-userinfo-proxy 전환. 비표준 userinfo 평탄화로 "missing provider id" 해결, email_verified 매핑으로 카카오/네이버 account linking 지원.
