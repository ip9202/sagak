...
| 2026-07-23 | #159 (87d686f develop) | fix(deploy) tag-release checkout fetch-tags 추가 (AC-DEPLOY-010 follow-up) | Deploy CI run #29909418596 failure 수정: `tag-release` job의 create-tag step이 `git rev-parse`(로컬 ref만 검사) + checkout `fetch-tags: false`로 remote tag 미인식 → `reference already exists` 거부. checkout에 `fetch-tags: true` 추가로 rev-parse가 remote tag 인식 → 안전 skip 분기(`tagged=false`). sync-auditor 독립 재검증 PASS-WITH-DEBT(9.23, 근본 원인 해결 YES). 비차단 follow-up 2종(ls-remote defense-in-depth, line-45 커밋 메시지 보간 hardening) 별도 추적. SPEC status는 `implemented` 유지(인프라 SPEC, post-merge follow-up fix). |
| 2026-07-23 | (본 PR — feature/SPEC-DEPLOY-001-deploy-hardening) | fix(deploy) tag-release injection hardening + ls-remote defense-in-depth (PR #159 비차단 follow-up 2종 처리) | PR #159가 남긴 비차단 follow-up 2종 해소: (1) **injection hardening** — `parse-version` step의 `${{ github.event.head_commit.message }}` 직접 보간(untrusted input 쉘 인젝션 위험)을 `env: COMMIT_MSG` 전달 후 `$COMMIT_MSG` 참조로 변경. (2) **ls-remote defense-in-depth** — `create-tag` step의 `git rev-parse`(로컬 ref만)를 `git ls-remote --tags origin`(remote authoritative)로 보강 + `git push` 실패 시 `already exists\|rejected` grep으로 safe-skip(`tagged=false`) 처리(cross-actor race window 닫기). 검증: `actionlint` PASS + YAML parse OK + 보안 review(잔존 `${{ }}` 보간 6곳 모두 안전 — concurrency group/job outputs/검증된 SemVer/secret env). SPEC status `implemented` 유지(인프라 SPEC, post-merge follow-up). |
