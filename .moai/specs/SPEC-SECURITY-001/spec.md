...
| 2026-07-03 | 1.1.0 | REQ-SEC-050 런타임 smoke 기반 정합성 정정 — (1) 알고리즘 핀 RS256→ES256 (실제 Supabase 토큰/JWKS = ES256/EC P-256). (2) 발행자 핀 SUPABASE_URL→`${SUPABASE_URL}/auth/v1` (실제 iss 클레임). RS256 자가 서명 단위 테스트가 프로덕션 ES256 경로를 전혀 exercised 하지 않아 놓친 결함 (lessons #4 case). | 강력쇠주먹 |
...
| (d3) 알고리즘 혼동 (ES256 vs HS256) | 공격자가 HS256 토큰으로 서명 검증 회유 | L0가 핀치 못하면 무력화 | **의미 있는 2차 방어** |
...
#### REQ-SEC-040: 알고리즘 ES256 고정

`verifyAndExtractJwtSub`는 **항상** `algorithms: ['ES256']` 옵션으로 `jwtVerify`를 호출해야 한다. 이는 HS256 혼동 공격을 차단한다. 실제 Supabase 액세스 토큰은 ES256(ECDSA P-256)으로 서명되며, JWKS `${SUPABASE_URL}/auth/v1/.well-known/jwks.json`은 단일 EC 키(`{alg:ES256, kty:EC, crv:P-256}`)를 반환한다 (REQ-SEC-050 런타임 smoke로 확정, 2026-07-03). 단일 ES256 핀은 alg 불일치 토큰(HS256 포함)을 즉시 거부한다.

#### REQ-SEC-041: 발행자(issuer) 고정

`verifyAndExtractJwtSub`는 **항상** `issuer: ${SUPABASE_URL}/auth/v1` 옵션으로 검증해야 한다. 실제 Supabase JWT의 `iss` 클레임은 `https://<project-ref>.supabase.co/auth/v1` 형태다 (REQ-SEC-050 런타임 smoke로 확정, 2026-07-03). `SUPABASE_URL`은 Deno 환경변수에서 읽고 `/auth/v1` 경로를 덧붙인다.
...
단위 테스트는 ES256(EC P-256) 키페어를 생성하여 서명한 유효 JWT가 `verifyAndExtractJwtSub`를 통과하여 `{ sub }`를 반환함을 검증해야 한다.
...
단위 테스트는 HS256 알고리즘으로 서명한 토큰(공격자가 공개 키로 HS256 서명을 시도)이 `null`을 반환함을 검증해야 한다. 이는 REQ-SEC-040의 실제 방어를 증명한다 — ES256 단일 핀이 alg 불일치를 즉시 거부한다.
...
| SPEC-AUTH-001 | JWT 발행 주체 (Supabase Auth, ES256) |
...
2. `logic.ts`에 `verifyAndExtractJwtSub` 헬퍼 추가 — `createRemoteJWKSet(new URL("${SUPABASE_URL}/auth/v1/.well-known/jwks.json"))` + `jwtVerify(token, JWKS, { algorithms: ['ES256'], issuer: \`${SUPABASE_URL}/auth/v1\`, audience: 'authenticated' })`
...
**테스트 (REQ-SEC-060~064)**: ES256(EC P-256) 키페어 생성(예: node crypto `generateKeyPairSync('ec', {namedCurve:'P-256'})` 또는 jose 자체 유틸), 유효/변조/HS256혼동/만료 토큰 케이스, JWKS fetch 인터셉트 모킹.
