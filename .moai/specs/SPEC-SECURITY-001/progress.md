...
- **REQ-SEC-052 회귀 검증(실기기) 완료 (2026-07-06)**: prod Pixel 6 kakao OAuth → kakao-book-search 책 검색 200 (jose 도입 후 기존 동작 유지 입증). send-notification/naver-userinfo-proxy는 코드 수준 입증 — 함수별 `deno.json` jose 출현 0회, index.ts jose 미참조; naver는 추가로 `verify_jwt=false` + deno.json 부재로 jose와 완전 직교. 무효 JWT 401은 A1 `verify-jwt-policy.sh` 매트릭스 정적 보장(`config.toml verify_jwt=true`). 검증 수단: curl→실기기 전환(사용자 선택) — kakao-book-search 실기기 직접, 나머지 코드 입증. AC 20/21 → 21/21 완료. lessons #19(회귀 검증(실기기) 표현 준수), #25~#29(런타임 smoke 패턴) 적용.
...
- **M5 (런타임 smoke test)**: ✅ 완료 — REQ-SEC-050(로컬 JWKS ES256 kid=`b81269f1` + serve iss 정합 입증, 2026-07-03) + REQ-SEC-051(프로덕션 실기기 smoke 입증 — Phase E 200, 2026-07-04) + REQ-SEC-052(타 보호 함수 회귀 검증(실기기) — kakao-book-search 200, 2026-07-06) 전부 완료. iss/aud/JWKS-URL 정확성 런타임 확정 완료.
...
| REQ-SEC-052 (다른 함수 회귀) | 완료 (prod 실기기 kakao-book-search 200 회귀 검증(실기기), 2026-07-06. send-notification/naver-userinfo-proxy 코드 수준 입증 — deno.json jose 0회 + verify_jwt 매트릭스. 무효 JWT 401은 A1 정적 보장) |
...
| ES256 정정 (2026-07-03) | 040,041 정정 (RS256→ES256 / issuer=`${SUPABASE_URL}/auth/v1`) | 17/21 | 0 | REQ-SEC-050 smoke로 프로덕션 초기 구현 결함 발견·수정. AC 카운트 변동 없음(이미 "완료" 라벨이었으나 실제로는 프로덕션 401 초기 구현 결함이었음). M5 런타임 smoke는 여전히 배포 게이트. |
...
| REQ-SEC-052 회귀 검증(실기기) (2026-07-06) | 052 완료 | 21/21 | 0 | prod Pixel 6 kakao OAuth → kakao-book-search 책 검색 200 (jose 도입 후 기존 동작 유지). send-notification/naver-userinfo-proxy 코드 수준 입증(함수별 deno.json jose 0회 + index.ts 미참조; naver는 verify_jwt=false + deno.json 부재로 jose와 완전 직교). 무효 JWT 401은 A1 verify-jwt-policy.sh 정적 보장. AC 21/21 완료 — SPEC-SECURITY-001 모든 AC 충족. |
...
- M5 런타임 smoke: REQ-SEC-050(로컬 JWKS/serve 정합) + 051(프로덕션 실기기 200/401) + 052(타 보호 함수 회귀 검증(실기기)) 전부 입증 완료 (052는 2026-07-06). iss/aud/JWKS-URL 런타임 확정. AC 21/21 완료 — 본 SPEC 모든 AC 충족. 런타임 smoke 시 보호 엔드포인트 최초 호출은 JWKS 콜드스타트로 transient 401 가능 → 반드시 재시도로 확정 (lessons #27). 실패 시 첫 의심 지점 = logic.ts `issuer` 클레임 (실제 Supabase JWT iss 디코딩으로 확인).
