...
- Supabase RLS: `notifications_select_own` (REQ-DB-021) — Realtime에도 동일 적용 검증 필요(§C 사전 검증).
...
# 3. Supabase Realtime 활성화 상태 확인 (Dashboard 또는 config)
#    - notifications 테이블의 publication/Realtime 활성화 여부
#    - RLS 정책이 Realtime 구독에도 적용되는지 (Supabase 공식 문서 교차 검증)
...
**[NEEDS CLARIFICATION: Supabase Realtime RLS 적용 범위]** — Realtime `postgres_changes` 채널이 RLS 정책을 자동 적용하는지, 별도 publication 구성이 필요한지 run-phase 사전 확인 필요 (lessons #10 "Realtime RLS는 publication 별도 구성" 참조). run-phase ANALYZE에서 Supabase 공식 문서 + 기존 프로젝트 Realtime 사용처(grep)로 확인 후 설계 확정.
...
작업:
- Supabase Realtime `postgres_changes` INSERT 이벤트 채널 구독 훅 작성 (`useNotifications` 진입 시 또는 신규 `useNotificationsRealtime` 훅 — ANALYZE에서 기존 구조 기반 결정).
- 화면 진입 시 구독 수립, unmount 시 cleanup(useEffect cleanup).
- 수신 이벤트 → `queryClient.setQueryData`로 캐시 갱신, 또는 `invalidateQueries` 트리거.
- RLS 적용 검증(§C [NEEDS CLARIFICATION] 해소) — 타인 알림 수신 안 됨 확인.
