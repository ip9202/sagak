...
- `src/features/feed/useClubFeedRealtime.ts` (131행)가 Realtime 구독 훅의 정준 패턴이다 — `client.channel(name)` → `.on('postgres_changes', {event, schema, table, filter}, cb)` (cb는 `queryClient.invalidateQueries` 호출) → `channel.subscribe(statusCb)` + cleanup `channel.unsubscribe()` + `client.removeChannel(channel)`. REQ-NOTIF2-001의 구독 훅은 이 패턴을 `notifications` 테이블에 그대로 준용한다.
- `supabase/migrations/20240620000001_enable_realtime_feed.sql`이 Realtime publication 구성 + RLS 브로드캐스트 게이트의 정준 선례이다 (`ALTER PUBLICATION supabase_realtime ADD TABLE ...` + `REPLICA IDENTITY FULL`). SPEC-NOTIF-002의 새 notifications publication 마이그레이션(§C.2)은 이 선례를 준용한다.
- Supabase RLS는 `notifications` 테이블에 이미 적용되어 있다(REQ-DB-021 `notifications_select_own`). 단, Realtime 브로드캐스트에 RLS가 자동 적용되려면 publication 구성이 별도로 필요하다(§C.2 결정 — 현재 `notifications`는 어떤 publication에도 포함되지 않음을 grep 관측).
...
**추가 제약**:
- 기존 `useMarkAsRead`/`useMarkAllAsRead`의 invalidateQueries 패턴을 재사용한다(신규 패턴 도입 금지).
- invalidateQueries는 푸시 수신 및 배너 클릭 콜백 내에서 동일 쿼리 키 prefix(`[NOTIFICATION_QUERY_PREFIX]`)로 수행한다.
...
- 타인 알림은 RLS에 의해 Realtime 이벤트 수신 단계에서 차단된다. `notifications` 테이블은 현재 어떤 Realtime publication에도 포함되어 있지 않으므로(grep 관측), 새 마이그레이션 `supabase/migrations/<timestamp>_enable_realtime_notifications.sql`에서 `ALTER PUBLICATION supabase_realtime ADD TABLE public.notifications` + `REPLICA IDENTITY FULL` 구성이 필요하다(publication 구성은 자동이 아님). 구성 후 기존 `notifications_select_own` RLS 정책(REQ-DB-021)이 브로드캐스트를 자동 게이트한다 — 이는 기존 feed Realtime 마이그레이션 `supabase/migrations/20240620000001_enable_realtime_feed.sql`이 `emotion_records`/`sticker_reactions`에 대해 검증한 동일 패턴이다(해당 마이그레이션 주석 인용: "Supabase Realtime 는 postgres_changes 브로드캐스트 시 테이블의 SELECT RLS 정책을 그대로 적용한다. 즉, 구독자는 자신이 SELECT 할 수 있는 행에 대한 이벤트만 수신한다"). lessons #10 "Realtime RLS는 publication 별도 구성" 확정.
