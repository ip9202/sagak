...
#### REQ-CLUBC-002: 매개변수 검증 (호출자 = host)

**IF** RPC 호출 시 `p_host_id <> auth.uid()` 인 경우,
**THEN** 시스템은 빈 결과 집합을 반환해야 한다. (다른 사용자의 host 모임 진도를 조회할 수 없다.)
**AND** 이 검증은 RPC 본문의 `WHERE clubs.host_id = p_host_id` 조건이 `auth.uid()` 와 무관하게
`p_host_id` 에만 의존하므로, 클라이언트가 타인의 UUID 를 넘기더라도 해당 UUID 가 host 인 모임만
반환된다. 단, 모임의 `clubs` SELECT RLS(`USING(true)`)로 인해 모든 authenticated 사용자가
어떤 모임이 누구의 host 모임인지는 알 수 있다. 따라서 host 본인 호출(`p_host_id = auth.uid()`)이
아닌 경우는 클라이언트가 실수로 잘못된 UUID 를 넘긴 경우이며, 이때 빈 결과를 반환한다.

> 보안 메모: 진도 데이터 자체는 `user_books_public` 뷰(`is_public=true` 행만)에서 오므로,
> 설령 타인의 `p_host_id` 로 호출해도 노출되는 것은 공개 설정된 독자의 진도뿐이다. 하지만
> 사용자 경험상 "자신의 host 모임만" 이 명확하도록 클라이언트는 항상 `auth.uid()` 를 전달한다.
