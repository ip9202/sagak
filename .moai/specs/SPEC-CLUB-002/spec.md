...
- **데이터베이스**: Supabase PostgreSQL — `clubs` (SPEC-DB-001 REQ-DB-006), `club_members` (SPEC-DB-001 REQ-DB-007)
...
1. Track B 모임은 **0명이어도 출발**한다. `min_members`는 게이트가 아니며 (pages_06 ERD 2.6,
   SPEC-DB-001 REQ-DB-006 주석), 개설 즉시 `status='active'`이고 host 1명으로 모임이
   활성화된다. 추후 멤버가 가입하지 않아도 모임은 유효하다.
...
**AND** INSERT 본문은 `book_id`(NOT NULL), `title`(NOT NULL), `type='group'`을 반드시
포함해야 한다.
...
**WHILE** 모임이 생성되는 동안,
**THEN** 시스템은 `min_members` 값을 최소 인원 게이트로 사용하지 않아야 한다.
`min_members`는 0이거나 NULL이어도 모임 생성이 성공하며, host 1명으로 모임이 즉시
활성화된다 (pages_06 ERD 2.6, SPEC-DB-001 가정 2.2.1).

#### REQ-CLUBB-004: 모임 설정 입력

**WHEN** 사용자가 모임 생성 폼을 작성하면,
**THEN** 시스템은 다음 설정 값을 선택적으로 수집하여 `clubs` INSERT에 포함해야 한다:
`description`(모임 설명), `duration_days`(목표 완독 기간), `daily_pages`(일일 권장 페이지),
`trigger_page`(트리거 페이지 — 특정 페이지 도달 시 이벤트). 이 값들은 NULL을 허용하며
(REQ-DB-006), host가 추후 진도 동기화(REQ-CLUBB-PROGRESS)에서 업데이트할 수 있다.

#### REQ-CLUBB-005: 모임 생성 결과 반환

**WHEN** `clubs` INSERT가 성공하면,
**THEN** 시스템은 생성된 `clubs` 행의 전체 객체(`id`, `host_id`, `book_id`, `type`,
`title`, `status='active'`, `started_at`, 기타 설정)를 `.select().single()`로 반환해야 한다.
