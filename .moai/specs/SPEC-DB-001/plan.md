...
| 날짜 | 버전 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| 2026-06-14 | 1.0.0 | 최초 구현 계획 — 16개 마이그레이션 순서 정의 | 강력쇠주먹 |
| 2026-06-14 | 1.2.0 | 감사 리포트 N1-N9 반영 — Option A(RLS 단독), 트리거 위치 정정, FK ON DELETE 정책 | 강력쇠주먹 |
| 2026-06-23 | 1.3.0 | 구현 완료 상태 반영 — 16개 코어 마이그레이션 + 후속 SPEC 확장 마이그레이션 분류, 브라운필드 유지보수 모드 전환, 잔여 검증 항목 정리 | 강력쇠주먹 |
...
> **현재 상태 (2026-06-23)**: 코어 구현(T-001 ~ T-009)은 완료되었다. 초기 16개
> 마이그레이션(`20240614*`)으로 12개 엔터티 + RLS + 보안 뷰 + 트리거가 구축되었으며,
> 272개 pgTAP 테스트가 100% 통과한다. 이후 SPEC-NOTIF-001, SPEC-ROUTINE-001,
> SPEC-FEED-001 등 도메인 SPEC 구현 과정에서 후속 마이그레이션
> (`20240618*`, `20240620*`)이 추가되었다. 본 plan은 향후 유지보수와 잔여 검증
> 항목을 안내한다.
...
Supabase CLI 규칙(`supabase/migrations/{YYYYMMDD}_{name}.sql`)을 따른다.
마이그레이션은 의존성 순서대로 정렬된다.

### 2.1 코어 마이그레이션 (SPEC-DB-001 범위 — 16개, 구현 완료)

| 순서 | 파일명 | 내용 | 의존성 | 상태 |
|------|--------|------|--------|------|
| 1 | `20240614000001_create_users.sql` | `users` 테이블 + `handle_new_user` 트리거(auth.users 연동, SECURITY DEFINER) | 없음 | DONE |
| 2 | `20240614000002_create_books.sql` | `books` 카탈로그 테이블 | 없음 | DONE |
| 3 | `20240614000003_create_user_books.sql` | `user_books` 서재 junction + UNIQUE + CHECK | users, books | DONE |
| 4 | `20240614000004_create_clubs.sql` | `clubs` 모임 테이블 + CHECK + `handle_new_club_host` 트리거(SECURITY DEFINER) | users, books | DONE |
| 5 | `20240614000005_create_emotion_records.sql` | `emotion_records` + CHECK(visibility/club_id) | users, books, clubs | DONE |
| 6 | `20240614000006_create_sticker_enum_and_reactions.sql` | `sticker_type` ENUM + `sticker_reactions` + UNIQUE | emotion_records, users | DONE |
| 7 | `20240614000007_create_club_members.sql` | `club_members` + UNIQUE + CHECK | clubs, users | DONE |
| 8 | `20240614000008_create_join_requests.sql` | `join_requests` + UNIQUE + CHECK + 상태 전환 트리거(BEFORE UPDATE RAISE + AFTER UPDATE ACCEPT→member 삽입, SECURITY DEFINER) | clubs, users, club_members | DONE |
| 9 | `20240614000009_create_reading_sessions.sql` | `reading_sessions` 타이머 세션 | users, books | DONE |
| 10 | `20240614000010_create_completion_reports.sql` | `completion_reports` + UNIQUE(user_book_id) + 완독 자동 생성 트리거(ON CONFLICT DO NOTHING, SECURITY DEFINER) | users, books, user_books | DONE |
| 11 | `20240614000011_create_point_logs.sql` | `point_logs` 포인트 내역 + CHECK | users | DONE |
| 12 | `20240614000012_create_notifications.sql` | `notifications` 알림 로그 + CHECK | users | DONE |
| 13 | `20240614000013_create_indexes.sql` | ERD 섹션 3의 모든 권장 인덱스 | 전체 | DONE |
| 14 | `20240614000014_enable_rls.sql` | RLS 활성화 + 모든 테이블 정책 (REQ-DB-013a ~ REQ-DB-021) + `fn_user_in_club` 헬퍼 함수(SECURITY DEFINER) | 전체 | DONE |
| 15 | `20240614000015_create_triggers.sql` | `updated_at` 자동 갱신 트리거 (users, user_books, emotion_records) | 전체 | DONE |
| 16 | `20240614000016_enable_books_rls.sql` | `books` 테이블 공개 카탈로그 SELECT 정책 (`USING (true)`, REQ-DB-013b) | books | DONE |

> **참고**: 보안 뷰 `user_profiles`, `user_books_public`(REQ-DB-013e)는 0014에 통합
> 생성되었다(독립 0015 파일은 없음 — 기능 동등성 확인됨). 0015 번호는 updated_at
> 트리거로 재사용되었다.

### 2.2 후속 SPEC 확장 마이그레이션 (SPEC-DB-001 파생 — 12개)

이 마이그레이션들은 SPEC-DB-001 코어 위에 도메인 SPEC 구현 중 추가된 것이다.
SPEC-DB-001 자체의 요구사항은 아니지만, 동일 스키마를 수정하므로 추적이 필요하다.

| 파일명 | 소스 SPEC | 내용 |
|--------|-----------|------|
| `20240614000017_users_nickname_check.sql` | SPEC-AUTH-001 | nickname 형식 검증 CHECK |
| `20240614000018_users_provider_naver.sql` | SPEC-AUTH-001 | provider CHECK에 'naver' 추가 (Custom OIDC) |
| `20240618000001_backfill_users.sql` | SPEC-AUTH-001 | 기존 users 행 보정 |
| `20240618000002_fix_handle_new_user_columns.sql` | SPEC-AUTH-001 | handle_new_user 트리거 컬럼 정정 |
| `20240618000003_users_nickname_nullable.sql` | SPEC-AUTH-001 | nickname NULL 허용 (온보딩 전) |
| `20240618000004_handle_new_user_custom_provider_strip.sql` | SPEC-AUTH-001 | provider 값 정규화 |
| `20240618000005_handle_new_user_email_fallback.sql` | SPEC-AUTH-001 | email fallback 로직 |
| `20240618000006_add_club_reading_plan_columns.sql` | SPEC-CLUB-001 | clubs 읽기 계획 컬럼 추가 |
| `20240620000001_enable_realtime_feed.sql` | SPEC-FEED-001 | Realtime publication 구성 (REQ-DB 범위 외, 별도 검증 필요 — memory lesson #10) |
| `20240620000002_create_reading_session_rpc.sql` | SPEC-ROUTINE-001 | 독서 세션 계산 RPC 함수 (SECURITY DEFINER) |
| `20240620000003_enrich_notifications_for_notif.sql` | SPEC-NOTIF-001 | notifications.type → ENUM 전환 + data jsonb + users.push_token |

> **주의**: `20240620000001_enable_realtime_feed.sql`은 Realtime publication 구성을
> 변경한다. Realtime은 RLS와 별도의 publication 권한 모델을 사용하므로(메모리
> 교훈 #10), 본 마이그레이션이 RLS 정책과 독립적으로 동작하는지 별도 검증이 필요하다.
...
4. **보안 뷰는 RLS 이후**: 뷰는 베이스 테이블 RLS 정책(0014)이 확정된 후 생성.
...
- 보안 뷰 `user_profiles`, `user_books_public` 생성 (0014 통합) — MUST-PASS
- 인덱스 생성 (서재/피드 조회용)
- 272개 pgTAP 테스트 100% 통과 (2026-06-14 검증)

### Milestone 2 — 소셜 연결 (DONE)
...
### Milestone 4 — 유지보수 검증 (Ongoing, Priority Low)

코어 구현은 완료되었으나, 운영 단계에서 다음 검증이 권장된다:

- **M4-1 Realtime publication 검증**: `20240620000001` 마이그레이션이 RLS와
  독립적으로 올바르게 동작하는지 확인 (메모리 교훈 #10 — Realtime RLS는
  publication 별도 구성). 본 검증은 SPEC-FEED-001 책임 영역이나 DB 계층
  영향이 있으므로 여기에 기록한다.
- **M4-2 notifications ENUM 전환 무결성**: `20240620000003` 마이그레이션이
  text → ENUM 전환 시 기존 행을 올바르게 매핑했는지 확인 (데이터 손실 가능성).
- **M4-3 프로덕션 RLS 침투 테스트**: 개발 환경 검증은 완료되었으나, 프로덕션
  Supabase에서 타 사용자 데이터 누출 시나리오에 대한 정기적 회귀 테스트 권장.

---

## 4. 위험 분석 (Risk Analysis)

### 위험 1: RLS 정책 오구성 = 데이터 유출 (CRITICAL — 완화 적용됨)
...
**잔여 검증**: 프로덕션 환경에서 주기적 RLS 회귀 테스트(M4-3) 권장.

### 위험 2: user_books 비즈니스 컬럼 기본값 오류 (HIGH — 완화 적용됨)
...
### 위험 5: auth.users와 public.users 동기화 누락 (MEDIUM — 완화 적용됨, 후속 패치 있음)

**위험**: `handle_new_user` 트리거가 실패하면 인증은 되었지만 프로필이 없는 사용자가 발생한다.

**완화 (적용 완료 + 후속 SPEC-AUTH-001 패치)**:
- 트리거를 `AFTER INSERT ON auth.users`로 설정
- `SECURITY DEFINER` 함수로 권한 보장 (REQ-DB-013c)
- SPEC-AUTH-001 구현 중 `handle_new_user` 컬럼/provider/email fallback 패치
  (마이그레이션 `20240618000002`, `...04`, `...05`) 적용 — 네이버 Custom OIDC
  연동 과정에서 발견된 결함 정정

### 위험 6: 컬럼 마스킹 실패로 민감 정보 노출 (CRITICAL — 해결됨)

**위험**: PostgreSQL RLS는 행수준만 지원하므로, 타인 행의 민감 컬럼(email,
reading_alarm_time 등)이 노출될 수 있다.

**완화 (감사 리포트 D1 + N1 해결 — 적용 완료)**:
...
**완화 (감사 리포트 D7 해결 — 적용 완료)**:
- `fn_user_in_club(p_club_id uuid) RETURNS boolean` SECURITY DEFINER 함수 정의
- 이 함수는 `BYPASSRLS` 속성의 소유자(`postgres` 슈퍼유저 역할)로 RLS를 우회하여 멤버십 판정
- emotion_records 및 club_members RLS 정책은 서브쿼리 대신 이 함수 호출 사용

### 위험 8: 후속 SPEC 마이그레이션이 코어 RLS를 훼손 (MEDIUM — 모니터링 필요)

**위험**: SPEC-NOTIF-001, SPEC-ROUTINE-001 등 도메인 SPEC 구현 중 추가된 마이그레이션이
코어 RLS 정책이나 SECURITY DEFINER 함수와 충돌할 수 있다.

**완화**:
- 후속 마이그레이션은 코어 16개 파일과 별도 타임스탬프(`20240618*`, `20240620*`)로 분리
- 각 도메인 SPEC run 시 코어 스키마에 대한 회귀 테스트(기존 272개 pgTAP) 재실행 권장
- Realtime publication(`20240620000001`)은 RLS와 별도 권한 모델이므로 독립 검증(M4-1)
...
- **Supabase CLI** (v2.104.0): 마이그레이션 실행 도구 (`supabase db push`, `supabase test db`)
...
- **pgTAP**: 마이그레이션 테스트 프레임워크 (`supabase/tests/*.sql`)

### 내부 의존성 (SPEC 간)

본 SPEC은 데이터 계층의 기반이므로, 다른 모든 SPEC의 전제 조건이다:
- SPEC-AUTH-001: `users` 테이블 + `handle_new_user` 트리거 의존 (후속 패치 6건 적용됨)
- SPEC-EMOTION-001: `emotion_records`, `sticker_reactions` 의존
- SPEC-CLUB-001 / SPEC-CLUB-002: `clubs`, `club_members`, `join_requests` 의존
- SPEC-ROUTINE-001: `reading_sessions` + RPC 함수 의존
- SPEC-COMPLETION-001: `completion_reports` 의존
- SPEC-FEED-001: `emotion_records` Realtime publication 의존
- SPEC-NOTIF-001: `notifications` ENUM 전환 의존
...
| 독서 세션 계산 (duration) | DB RPC 함수 | 예 | 서버 시간 기준 원자적 계산 (SPEC-ROUTINE-001) |
...
코어 구현은 완료되었다. 향후 작업은 다음 원칙을 따른다:

### 7.1 새 컬럼/제약 추가 시

- 항상 새 타임스탬프 마이그레이션 파일을 생성한다 (기존 파일 수정 금지).
- 후속 SPEC 확장 마이그레이션 패턴(`20240618*`, `20240620*`)을 참고한다.
- 백필이 필요한 경우 별도 backfill 마이그레이션으로 분리한다.

### 7.2 RLS 정책 변경 시

- 변경 후 반드시 `supabase test db`로 272개 기존 테스트가 회귀 없는지 확인한다.
- 새 정책은 전용 테스트 파일(`supabase/tests/NNNN_*.sql`)을 추가한다.
- SECURITY DEFINER 함수 소유자는 `postgres` 슈퍼유저 역할을 유지한다 (N9).

### 7.3 ENUM 확장 시

- `sticker_type` ENUM 값 추가: `ALTER TYPE ... ADD VALUE` 마이그레이션 필요 (D13).
- `notification_type` ENUM(SPEC-NOTIF-001) 값 추가 시에도 동일 패턴.

### 7.4 Realtime publication 변경 시

- RLS와 별도 권한 모델이므로(메모리 교훈 #10), publication 변경 시 RLS 정책과의
  상호작용을 별도 검증한다 (M4-1).
...
> **현재 결정**: 단일 SPEC-DB-001로 유지. 코어 구현 완료 후 도메인 SPEC들이
> 파생 마이그레이션을 추가하는 패턴이 정착되었다.
...
| `.moai/specs/SPEC-DB-001/progress.md` | 태스크 실행 로그 (T-001 ~ T-009 완료) |
| `.moai/specs/SPEC-DB-001/tasks.md` | 태스크 분해 상세 |
