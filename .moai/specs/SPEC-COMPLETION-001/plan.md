...
| 옵션 | 라이브러리 | 장점 | 단점 |
|------|-----------|------|------|
| A (권장) | 순수 SVG (react-native-svg) | 의존성 최소, 커스터마이징 자유 | 구현 비용 중간 |
| B | react-native-chart-kit | 빠른 구현, 기본 차트 제공 | 커스터마이징 제한 |
| C | victory-native | 고급 기능, 풍부한 차트 | 의존성 무거움, 러닝 커브 |

> **결정 시점**: RUN 단계에서 프로토타이핑 후 확정. 감정 곡선 데이터 구조가 단순하므로(page-count 쌍) 옵션 A가 우선 후보다.

### 런타임 타입 검증 (report_data 파싱)

| 옵션 | 라이브러리 | 장점 |
|------|-----------|------|
| A (권장) | Zod | 스키마 정의 + 타입 추론 동시 제공, React Native 호환 |
| B | 순수 타입 가드 함수 | 의존성 없음, 수동 구현 |

> report_data 스키마가 고정되어 있으므로(3개 키), Zod 오버헤드가 작다. 옵션 A 권장.
...
| `src/features/completion/types.ts` | 신규 | ReportData, EmotionCurvePoint, Highlight 인터페이스 + Zod 스키마 | REQ-COMP-004 |
...
| `src/features/completion/EmotionCurveChart.tsx` | 신규 | 감정 곡선 차트 컴포넌트 (미결정 라이브러리 기반) | REQ-COMP-006 |
...
| 파일 | 수정 내용 | 협력 SPEC |
|------|----------|-----------|
| `app/(tabs)/library/[bookId]/completion.tsx` (또는 유사 라우트) | 완독 다이어리 라우트 추가 | SPEC-NAV-001, SPEC-LIBRARY-001 |
| `src/features/library/*.tsx` (서재 상세 화면) | 완독 상태 시 다이어리 진입 버튼 추가 | SPEC-LIBRARY-001 |
...
- `types.ts` — ReportData 인터페이스 + Zod 스키마 (REQ-COMP-004)
- `useCompletionReport.ts` — 조회 hook, 로딩/에러/빈 상태 분기 (REQ-COMP-005)
- 단위 테스트 — API 재시도, RLS 검증, 빈 상태 처리
...
- `EmotionCurveChart.tsx` — 감정 곡선 차트 (미결정 라이브러리 확정 후 구현, REQ-COMP-006)
...
### Optional Goal (선택) — 경량 애니메이션

- 미결정 사항 6.3 옵션 B/C 검토 후, 시간 허용 시 경량 애니메이션 추가
- MVP 기본은 옵션 A(정적 텍스트 + 배지)
...
completionApi.fetchReport(userBookId)
  ↓ (Zod 파싱 + 타입 가드)
useCompletionReport() → { data, isLoading, isError, isEmpty }
  ↓ (조건부 렌더링)
...
| 에러 (스키마 불일치) | Zod 파싱 실패 | "데이터 오류가 발생했어요" + 에러 로깅 |
...
- **위험**: 선택한 차트 라이브러리가 Expo SDK 55 또는 React 19.2와 호환되지 않을 수 있음
- **대응**: 미결정 사항 6.1을 RUN 단계 초기에 프로토타이핑으로 해결. 호환성 문제 시 순수 SVG(옵션 A)로 폴백.

### 리스크 3: report_data 스키마 진화

- **위험**: SPEC-DB-001 트리거 로직이 변경되어 report_data 구조가 달라질 수 있음
- **대응**: Zod 스키마로 런타임 검증 수행. 스키마 불일치 시 "데이터 오류" 상태로 안전하게 폴백 (REQ-COMP-004).
...
- `useCompletionReport.test.ts`: 로딩/성공/에러/빈 상태 분기
- `types.ts` Zod 스키마 테스트: 정상 데이터, 키 누락, 타입 불일치 케이스
...
- **자문 영역**: 차트 컴포넌트 아키텍처, React Native 성능 최적화(대량 하이라이트 렌더링), 접근성(a11y)
- **자문 시점**: Secondary Goal 시작 전 (차트 라이브러리 확정 시)

### 디자인 전문가 (design-uiux)

- **자문 영역**: 감정별 색상 토큰 설계, 빈 상태/에러 상태 UX, 축하 애니메이션 범위(미결정 6.3)
