...
| **SPEC-COMPLETION-001** | Completion Diary Visualization | ✅ Complete (2026-06-17) | src/features/completion/ (7 files + 4 tests), completionApi(fetchReport, PostgREST GET 래퍼 + 재시도 최대3 + 점진백오프 + normalizeError, RLS auth.uid() 신뢰), useCompletionReport(useState/useEffect 기반 6상태 훅: loading/success/empty/error/data-error/auth), EmotionCurveChart(순수 SVG, 단일 brand-500 색상, 페이지별 수량), HighlightList(FlatList, text.inverse), CelebrationHeader(정적 배지 MVP), CompletionDiaryScreen(6상태 분기 렌더링), types(isReportData 순수 타입 가드), 10 REQ coverage (REQ-COMP-001~010), 683/683 tests pass, 91.92% coverage |
| **SPEC-ROUTINE-001** | Reading Routine & Timer | ✅ Complete (2026-06-20) | src/features/routine/ (13 files + 10 tests), sessionApi(startSession/endSession RPC + getActiveSession), alarmApi(getAlarmSettings/updateAlarmTime/toggleAlarmEnabled, HH:MM→HH:MM:SS 정규화), statsApi(클라이언트 집계: total_duration/sessions/today/streak), streakCalculator(자정 기준 streak 계산), goalStorage(AsyncStorage 일일 목표), useReadingTimer(started_at 기반 setInterval 1초 + AppState 복귀 재동기화 + cleanup), useActiveSession/useAlarmSettings/useReadingStats(React Query v5 훅), copy(다정한 메시지), components(TimerScreen/AlarmScreen/RoutineStatsWidget), 10 REQ coverage, 83 tests pass (단위) + pgTAP 9/9 (통합), 88.54% stmts / 83.52% branches |
