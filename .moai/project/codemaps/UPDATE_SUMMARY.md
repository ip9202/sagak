...
### 2. entry-points.md
**변경 내용:**
- ✅ **신규 섹션:** Build-Time Entry Point (`app.config.ts`) 추가 — 빌드 시점 validateEnv 게이트, 빌드 시점 환경 변수 노출, 런타임 진입점과의 역할 분리 명시
...
- `app.config.ts`: validateEnv(process.env, ENV) 호출 + 빌드 시점 환경 변수을 extra에 노출
- `eas.json`: 3개 빌드 프로필 (development/preview/production)
- `.env.example`/`.env.staging`/`.env.production`: 환경 변수 플레이스홀더 플레이스홀더
...
### 미완료 마일스톤 (6차 기준, 2026-06-17)
- M2 — GitHub Actions CI (ci.yml/deploy.yml) — 미착수
- M3 — 통합 제거 (사용자 결정, 2026-07-07 PR #140)
