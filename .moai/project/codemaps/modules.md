...
| 모듈 | 경로 | 계층 | 목적 | SPEC 참조 |
|------|------|------|------|-----------|
| App Config | `app.config.ts` | Infrastructure (빌드) | Expo 앱 설정 — 빌드 시점 `validateEnv(process.env, ENV)` 호출(환경 변수 fail-fast 게이트, REQ-DEPLOY-018) | DEPLOY-001 REQ-DEPLOY-018/024 |
...
| Env Templates | `.env.example` / `.env.staging` / `.env.production` | Infrastructure (빌드) | 환경 변수 템플릿 | DEPLOY-001 REQ-DEPLOY-018/024 |
