...
| user_books | 개인 서재 (진행률/완독) | UNIQUE(user_id, book_id), status CHECK(reading/completed/shelved), is_public, 부분 UNIQUE(user_id WHERE status='reading'), status 기본값 'shelved' (정책 5.5 reading 단일) |
...
| notifications | 알림 | type notification_type ENUM(6종), data jsonb, is_read |
...
| user_books | user_books_one_reading_per_user | reading 단일 정책 부분 UNIQUE (user_id WHERE status='reading', 동시성 23505 최종 방어) |
...
| user_books | enforce_single_reading | BEFORE INSERT OR UPDATE OF status | reading 단일 정책(정책 5.5) — 한 사용자의 reading 행을 최대 1개로 강제, 새 reading 발생 시 기존 reading을 shelved로 배타 전환 |
...
## RPC Functions (3, SPEC-ROUTINE-001 + SPEC-CLUB-003)

| Function | 목적 | SECURITY | 인자 | 반환 | Note |
|----------|------|------------------|------|------|------|
| start_reading_session | 독서 타이머 시작 (자동종료+INSERT) | DEFINER (user_id=auth.uid() 가드) | p_user_book_id uuid | uuid | started_at=now(), ended_at=NULL |
| end_reading_session | 독서 타이머 종료 (서버 duration 계산) | DEFINER (user_id=auth.uid() 가드) | p_session_id uuid, p_pages_read int? | void | duration_seconds = EXTRACT(EPOCH FROM (ended_at - started_at)) |
| get_host_clubs_progress | 모임 진도 median 집계 (current_page>0) | INVOKER (auth.uid() defense-in-depth) | p_host_id uuid | TABLE(club_id, median_page, member_count_with_progress, total_pages) | user_books_public 뷰 소스, percentile_cont(0.5) median, books.total_pages LEFT JOIN |

**SECURITY 보안:**
- **DEFINER** (start/end_reading_session): RPC 함수는 RLS를 우회하므로 `user_id=auth.uid()` 가드(COERCE 기본값)로 명시적으로 본인 행만 조작. pgTAP 0018 테스트로 RLS 차단 검증 완료.
- **INVOKER** (get_host_clubs_progress): `user_books_public` 뷰가 이미 authenticated SELECT 권한을 보유하므로 권한 상승 불필요. auth.uid() 단정문으로 defense-in-depth (club_members RLS 보강). pgTAP 0019 8/8 PASS.
...
*Synced 2026-06-14 from supabase/migrations/0001-0015 (SPEC-DB-001 T-001~T-009). Updated 2026-06-18: migration 0004+0005 (SPEC-AUTH-001 naver Custom OIDC — handle_new_user custom: 정규화 + owner 고정 + email COALESCE 폴백). Updated 2026-06-19: migration 0006 (SPEC-CLUB-002 — clubs 진도 계획 컬럼 추가). Updated 2026-06-20: migration 20240620000001 (SPEC-FEED-001 — Realtime publication 활성화 + REPLICA IDENTITY FULL, Realtime 섹션 추가). Updated 2026-06-27: migration 20240627000001 (SPEC-CLUB-003 — get_host_clubs_progress RPC, SECURITY INVOKER, user_books_public 뷰 소스, median 집계). Updated 2026-06-30: migration 20240630000001 (SPEC-LIBRARY-001 — reading 단일 정책, 부분 UNIQUE 인덱스 user_books_one_reading_per_user + enforce_single_reading 트리거 + status 기본값 shelved).*
