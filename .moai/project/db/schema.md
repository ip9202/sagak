...
| Table | 목적 | 주요 제약 |
|-------|------|----------|
| users | 사용자 프로필 (auth.users 1:1) | email UNIQUE, role CHECK(member/admin), provider CHECK(kakao/apple/google) |
...
| clubs | 독서 모임 | type CHECK(group/instant), status CHECK(active/closed) |
