...
- **마이페이지·설정**: 사용자 프로필(Profile 타입 전체 컬럼), 독서 통계(완독/누적시간/감정기록), 포인트 내역, 배지(총건수 기준), 알림 설정 진입, 개인정보 처리, 이용약관 — SPEC-PROFILE-001 구현 완료 (2026-06-20 PR #36 e616614)
...
- **`src/features/feed/`** (모임 피드 + Realtime, SPEC-FEED-001 완료 — 2026-06-20): `types.ts` (FeedItem, StickerAgg, FeedQueryResult — emotion types 재사용 import), `spoilerFilter.ts` (클라이언트 측 스포일러 결정 순수 함수 — page_number vs current_page 비교, 본인 기록 항상 safe), `queries.ts` (PostgREST select 빌더 — club_id/visibility/book_id 필터 + 복합 커서 페이지네이션, users 조인 + sticker GROUP BY), `useClubFeed.ts` (피드 조회 훅 — useInfiniteQuery 커서 페이지네이션), `useClubFeedRealtime.ts` (Realtime 구독 훅 — postgres_changes 2리스너[emotion_records INSERT + sticker_reactions INSERT], invalidate + cleanup + SUBSCRIBED/CHANNEL_ERROR/TIMED_OUT 상태 추적, 재연결 시 전체 재조회), `components/ClubFeedScreen.tsx` (FlatList + EmotionRecordCard + 세션 revealed state + 빈/로딩/에러 + Realtime 연결 상태 배지), `index.ts` (통합 진입점). 라우트: `app/(tabs)/clubs/[clubId]/feed.tsx` + 모임 상세 CTA(`[clubId].tsx`)
...
- **`src/features/notification/`** (푸시 알림 및 알림 센터, SPEC-NOTIF-001 완료 — 2026-06-21 PR #34+38): `types.ts` (Notification, NotificationType — 6종 type ENUM), `queries.ts` (PostgREST 쿼리 — 목록/읽음/읽지 않은 카운트), `useNotifications.ts` (알림 목록 훅), `useUnreadCount.ts` (배지 카운트 훅), `markAsRead.ts` (개별 읽음 처리), `markAllAsRead.ts` ("모두 읽음" 배치), `routeMapper.ts` (type별 딥링크 매퍼), `registerToken.ts` (토큰 서버 등록 — users.push_token UPDATE, RLS 단일진실), `registerForPush.ts` (Expo Push Token 획득 + 권한 요청 — 실패 시 null silent), `usePushTokenRegistration.ts` (인증 시 1회 등록 멱등), `useNotificationResponse.ts` (포그라운드 핸들러 + 탭 라우팅), `index.ts` (통합 진입점 — 13 modules barrel)
...
- `GET /clubs/{id}/feed` - 진도별 슬라이딩 피드 (현재 진도 기반 스포일러 블러 처리, Supabase Realtime 구독과 병행) — SPEC-FEED-001 구현 완료 (2026-06-20, `src/features/feed/queries.ts`). 커서 기반 페이지네이션(`created_at` DESC, 임시 페이지 크기 20행), club_id/visibility/book_id 필터, 작성자 프로필 + 스티커 집계 포함. Realtime: `postgres_changes` on `emotion_records`(INSERT) + `sticker_reactions`(INSERT) — migration `20240620000001_enable_realtime_feed.sql`로 publication ADD + REPLICA IDENTITY FULL 적용
...
- `POST /functions/process-join-request` - 가입 요청 처리 (이중 방어선: 게이트웨이 `verify_jwt=true` + 앱 단 `verifyAndExtractJwtSub` jose ES256/JWKS 서명 검증, SPEC-SECURITY-001 PR #121/#123)
