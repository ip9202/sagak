...
### 데이터 페칭 계층 (SPEC-LIBRARY-001 — 2026-06-16)

TanStack React Query v5를 서버 상태 관리 라이브러리로 채택했다. QueryClientProvider는 `app/_layout.tsx` 최상위에서 AuthProvider/ThemeProvider 내부에 배치되어 전역 쿼리 클라이언트를 제공한다. `src/lib/query/queryClient.ts` 싱글톤은 defaultOptions로 3분간 staleTime, retry 1회, mutations retry 없음을 설정한다. 모든 API 호출은 useQuery/useMutation 훅을 통해 자동 캐싱/동기화/재시도를 얻으며, DevTools로 쿼리 상태를 디버깅할 수 있다. gen-types는 `npm run gen-types` 스크립트(`scripts/gen-types-with-header.js`)로 실행하며 로컬 Supabase에 연결하여 816라인의 `Database` 타입을 생성하고 헤더를 보존한다. Supabase 클라이언트는 `createClient<Database>` 제네릭으로 REQ-API-007 타입 안전을 보장한다.
